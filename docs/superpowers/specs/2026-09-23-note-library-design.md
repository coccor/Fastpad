...
- **Deleting to the Recycle Bin** (§8.4) uses `SHFileOperationW` with `FOF_ALLOWUNDO` instead of `IFileOperation`. It gives the same behavior with no COM vtable. `recycle` takes only an absolute path to a file; a relative path or a directory is refused before anything is sent to the shell. The shell still warns before a permanent delete, for example when there is no Recycle Bin to send to (a network share, the bin disabled or over quota); that warning is owned by the FastPad window. A dirty tab is not autosaved before its file is deleted: the confirmation then reads “Move “name” to the Recycle Bin and discard unsaved changes?”.
...
- **Rescan merges** (§7.5): changes flushed to `library.ini` while a scan is running are kept; if another PC changed `library.ini` during the interval, one stat of the file decides which side is current; notes created or renamed while a scan is running stay in the index. The merge takes the scan's own note list and re-checks on disk only the paths FastPad itself added, removed or renamed since that scan started, so a bulk change made in Explorer costs no extra stat on the UI thread.
- **The local file** (§6.3) is written by the scan worker, scan cache included, before it posts `LIBRARY_READY`, and not at all when its text is unchanged. The UI thread writes it only when the recent list, the autosave switch or the missing times changed: from a copy of the state, on a one-off writer thread, with writes numbered so an older copy never lands after a newer one. At window close it is written on the UI thread, since a writer thread might not finish before the process exits.
- **A `library.ini` that cannot be read right now** (§7.6), for example a sharing violation while OneDrive syncs it, is not treated as unreadable. Each read takes the file's stamp before and after the bytes and retries up to three times when they differ. A load that meets a busy file leaves organizing off with a notice until the next rescan reads it again, and a rescan that meets one keeps the live library. A flush that must re-read a busy file keeps its pending operations and retries every 2 seconds without a notice; at window close it retries three times, 100 ms apart. Only a file that was read and has a missing or unknown version, or cannot be parsed, is unreadable.
- **The startup folder** (§7.1): the UI thread reads only `folders.ini`. Whether the command-line path or the most recent folder exists, and the fallback to `Documents\FastPad` with its notice, are decided on the worker, so an offline mapped drive cannot stall the window. Until the worker answers, the name box saves into the most recent folder.
- **Folder paths are normalized** (absolute, no trailing separator or `.` parts) before they key the local file or enter the recent list, so `D:\Notes\` and `D:\Notes` are one folder.
- **An open tab follows a note moved outside FastPad** (§7.3) by its stored path, compared without touching the disk. When the moved file's size and write time equal what the tab knew, autosave carries on (and resumes if it had paused because the old file vanished); otherwise the next autosave pauses on the changed file.
- **Switching folders** (§5) first autosaves the old folder's dirty notes. **Turning notes mode off** always applies; the pending metadata is flushed, retried once, and if it still cannot be written a notice says “Metadata changes could not be written to <folder>\.fastpad\library.ini”. An open name box closes.
- **Untitled labels** (§8.1) are kept up to date with notes mode off too (only edits at or above the label's line recompute them) and shown only with notes mode on, so turning it on labels every untitled tab without switching to it. Restored and recovered untitled tabs get their label from their text as they open.
- **First save and rename treat typed extensions differently** (§8.2, §8.4). The first save keeps a typed extension only when it is a note extension, so “v1.2 plan” and “build.ps1” each stay one name and get the tab's extension; this deliberately narrows “typing an extension in the box overrides it”. Note: Rename keeps the file's own kind: a name ending in its current extension (any case) splits there, so the prefilled `script.py` stays `script.py`; otherwise a typed note extension is taken; otherwise the current extension is kept, and an extensionless file stays extensionless unless an extension is typed.
